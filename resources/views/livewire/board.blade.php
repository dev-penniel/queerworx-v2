...
@php
    $boardMembers = \App\Models\TeamMember::where('is_active', true)
        ->where('member_type', 'board')
        ->orderBy('sort_order')
        ->latest()
        ->take(7)
        ->get();
    ...
    $boardRows = [
        $boardMembers->slice(0, 3),
        $boardMembers->slice(3, 2),
        $boardMembers->slice(5, 2),
    ];
@endphp
    ...
    <section class="relative overflow-hidden py-20 sm:py-24">
        ...
        <div class="relative mx-auto max-w-7xl px-6">
            ...
            <div class="mt-14 space-y-8">
                @forelse ($boardRows as $rowIndex => $row)
                    @if ($row->isNotEmpty())
                ...
                @empty
                    <div class="rounded-[8px] border border-white/10 bg-white/[0.05] p-8 text-center text-white/60">
                        Board member profiles will appear here once they are added in the admin dashboard.
                    </div>
                @endforelse
            </div>
        </div>
